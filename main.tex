...
\begin{frame}{Regressione lineare}
    \begin{figure}
        ...
        \begin{minipage}{0.4\textwidth}
            Dove $\lbrace x_i \rbrace_{i}$ dati di input e $\lbrace y_i \rbrace_{i}$ dati di output. Noi vogliamo trovare la retta $y = w\cdot x + b$ che meglio approssima i dati.
        \end{minipage}
        \end{figure}  
\end{frame}
...
\begin{frame}{Regressione lineare}
    ...
    \begin{tikzpicture}
		...
		% Bias node
		\node[box, above=0.8cm of sum] (bias) { $ b $ };
...
\begin{tikzpicture}
		...
		% Input nodes
		\node[box] (x1) { \( x_1 \) };
		\node[box, below=0.4cm of x1] (x2) { \( x_2 \) };
		\node[below=0.2cm of x2] (dots) { \vdots };
		\node[box, below=0.2cm of dots] (xn) { \( x_n \) };
		...
		% Output node
		\node[box, right=1cm of sum] (y) { \( \hat{y} \) };
		...
		\node[below=0.1cm of xn] { \textit{Input features} };
...
\begin{frame}{Regressione lineare}
    ...
    \end{tikzpicture}%
        ...
        Dove ogni freccia rappresenta una connessione a cui è associato un peso, ed ogni cerchio rappresenta un neurone, a cui è associato un bias.
        \end{minipage}
        \end{figure}  
\end{frame}
...
\begin{frame}{Multi-layer Perceptron}
    ...
    \begin{tikzpicture}[
        ...
        % Draw the hidden layer nodes
        \foreach \name / \y in {3}
			\node[neuron, fill=green!50, label=above:{$x_{\name}^{1}$}, label=below:{$b_{\name}^{1}$}] (H1-\name) at (2.5,-\y*1.5) {};
        \foreach \name / \y in {1, 2, 4}
            \node[neuron] (H1-\name) at (2.5,-\y*1.5) {};
        ...
        \foreach \source in {1,...,3}
            \foreach \dest in {3}
				\path (I-\source) edge[draw=green] node[midway, above, sloped, text=green] {$w_{\dest\source}$} (H1-\dest); 
...
\begin{frame}{Multi-layer Perceptron}
    ...
    \begin{tikzpicture}[
        ...
        \foreach \source in {1,...,3}
            \foreach \dest in {3}
				\path (I-\source) edge[draw=green] (H1-\dest); 
        \foreach \source in {1,...,3}
            \foreach \dest in {4}
...
\begin{frame}{Multi-layer Perceptron}
    ...
    \begin{tikzpicture}[
        ...
        \foreach \source in {1,...,4}
            \foreach \dest in {3}
				\path (H1-\source) edge[draw=green] (H2-\dest); 
        \foreach \source in {1,...,4}
            \foreach \dest in {4}
...
\begin{frame}{Multi-layer Perceptron}
    ...
    \begin{tikzpicture}[
        ...
        \foreach \source in {1,...,4}
            \foreach \dest in {3}
				\path (H2-\source) edge[draw=green] (O-\dest); 
    \end{tikzpicture}%
\end{minipage}%
\hfill
\begin{minipage}{0.35\textwidth}
